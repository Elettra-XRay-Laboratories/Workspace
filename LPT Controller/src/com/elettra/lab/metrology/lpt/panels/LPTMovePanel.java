...
public final class LPTMovePanel extends MovementListener implements ActionListener
{
	...
	private void manageEventSet() throws CommunicationPortException
	{
		try
		{
			...
			{
				CommandsFacade.executeCommand(
				    CommandsFacade.Commands.POS,
				    new CommandParameters(this.axis, DriverUtilities.parseSign((String) this.signComboBox.getSelectedItem()), Double.parseDouble(this.position
				        .getText()), ListenerRegister.getInstance()), this.port);
			}
		}
		catch (NumberFormatException exception)
		{
	...
	private void manageEventRunPlus() throws CommunicationPortException
	{
		CommandsFacade.executeCommand(CommandsFacade.Commands.RUN,
		    new CommandParameters(this.axis, DriverUtilities.getPlus(), 0.0, ListenerRegister.getInstance()), this.port);
	}

	private void manageEventFastPlus() throws CommunicationPortException
	{
		CommandsFacade.executeCommand(CommandsFacade.Commands.FAST,
		    new CommandParameters(this.axis, DriverUtilities.getPlus(), 0.0, ListenerRegister.getInstance()), this.port);
	}

	private void manageEventStepPlus() throws CommunicationPortException
	{
		CommandsFacade.executeCommand(CommandsFacade.Commands.STEP,
		    new CommandParameters(this.axis, DriverUtilities.getPlus(), 0.0, ListenerRegister.getInstance()), this.port);
	}
	...
	{
		CommandsFacade.executeCommand(CommandsFacade.Commands.RUN,
		    new CommandParameters(this.axis, DriverUtilities.getMinus(), 0.0, ListenerRegister.getInstance()), this.port);
	}
	...
	class MoveListener extends Thread
	{
		...
		public MoveListener(LPTMovePanel panel)
		{
			this.panel = panel;
		}
	...
	class EnableThread extends Thread
	{
		...
		public EnableThread(LPTMovePanel panel, boolean enabled)
		{
			this.panel = panel;
