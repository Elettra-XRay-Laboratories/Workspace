...
public class Omega2ThetaScanWindow extends AbstractGenericFrame
{
	...
	public static synchronized Omega2ThetaScanWindow getInstance(ICommunicationPort port) throws HeadlessException, IOException
	{
		return new Omega2ThetaScanWindow(port);
	}

	private Omega2ThetaScanWindow(ICommunicationPort port) throws HeadlessException, IOException
	{
		super("Omega-2Theta Scan", port);
		...
		gbl_leftPanel.rowHeights = new int[] { 440, 440 };
		...
		movePanel7.add(new DoubleMovePanel(Axis.OMEGA2THETA, this.getPort()));
		...
		scan0Panel.add(new I0ScanPanel(Axis.OMEGA2THETA, this.getPort(), false));
		...
		gbc_buttonsPanel.insets = new Insets(10, 5, 400, 10);
